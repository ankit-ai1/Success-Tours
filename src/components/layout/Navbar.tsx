      ...
      <nav className="container-max flex h-[72px] items-center justify-between">
        <Link to="/" className="flex items-center gap-2">
          ...
          <span className="flex flex-col leading-none">
            <span className={cn("font-display text-lg font-bold", solid ? "text-primary" : "text-white")}>Success Tours</span>
            <span className={cn("text-[10px] font-semibold uppercase tracking-[0.18em]", solid ? "text-secondary" : "text-accent")}>& Travel Kenya</span>
          </span>
        </Link>
        ...
        <div className="hidden items-center gap-3 lg:flex">
          <Button to="/quote" variant={solid ? "ghost" : "white"} size="sm">Request Quote</Button>
          ...
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }} exit={{ opacity: 0, height: 0 }}
            ...
            <ul className="container-max flex flex-col py-4">
              ...
              <li className="mt-4 flex gap-3">
                <Button to="/quote" variant="ghost" size="sm" fullWidth>Request Quote</Button>
